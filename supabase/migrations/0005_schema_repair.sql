-- =========================================================
-- Hapolonio Arte — SCRIPT DE VERIFICAÇÃO E CORREÇÃO
-- =========================================================
-- Rode este arquivo inteiro no SQL Editor do Supabase.
-- Ele é seguro de rodar quantas vezes precisar: só cria o que
-- ainda não existe e só adiciona colunas que estão faltando.
-- Resolve especificamente o erro:
--   "Could not find the 'mobile_image_url' column of 'banners'"
-- e qualquer outro campo que o site espera e o banco ainda não tem.
-- =========================================================
...
-- Remove colunas antigas que não existem mais no código
alter table public.site_settings
...
-- ---------------------------------------------------------
-- BANNERS — modelo atual: 1 registro = 1 nome + imagem desktop + imagem mobile
-- Se a tabela existir num formato antigo (com "device"/"image_url" únicos),
-- recriamos do zero: ainda é o início do projeto, então é seguro.
-- ---------------------------------------------------------
do $$
begin
...
-- ---------------------------------------------------------
-- SEED DE CATEGORIAS (só entra se ainda não existir a slug)
-- ---------------------------------------------------------
